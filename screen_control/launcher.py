...
def build_modules():
    ...
    subprocess.run(
        "nasm -f elf64 assembly/hook.asm -o assembly/hook.o", shell=True, check=True
    )
    subprocess.run(
        "ld -shared -o assembly/hook.so assembly/hook.o", shell=True, check=True
    )
...
def run_demo(controller):
    ...
    focus_cmd = {"action": "window_focus", "params": {"window_title": "Terminal"}}
    ...
    move_cmd = {"action": "mouse_move", "params": {"x": 10, "y": 10}}
