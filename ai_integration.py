...
class OllamaClient:
    ...
    async def generate(
        ...
        if not await self.ensure_service_running():
            self.logger.error(
                "Ollama service is not running and could not be auto-started."
            )
            return None
        ...
        if not model:
            ...
            if not available:
                self.logger.error(
                    "No models found in Ollama. Please pull a model using 'ollama pull <model>'."
                )
            else:
                self.logger.error(
                    f"Configured models ({self.main_model}, {self.assistant_model}) not found."
                )
            return None
