                    ...
                    if "ai_analysis" in data and data["ai_analysis"]:
                        ...
                        if isinstance(ai_data, dict):
                            summary = ai_data.get("summary") or \
                                      ai_data.get("tech_stack_assessment") or \
                                      "Analysis data available (click to view)"
...
def show_web():
    ...
    target = st.text_input("Enter Target Domain", placeholder="example.com", key="web_target")
    ...
    if st.button("🚀 Start Web Discovery"):
        if not target:
            ...
            async def run_discovery():
                ...
                results = await st.session_state.web.run_web_discovery(target, use_ai=use_ai)
                return results
            ...
            with st.spinner(f"Discovering {target}..."):
                ...
                if results.get("errors"):
                    ...
                    if use_ai and results.get("ai_analysis"):
                        ...
                        st.markdown(f"**Tech Stack Assessment:** {analysis.get('tech_stack_assessment', 'N/A')}")
...
def show_scan():
    ...
    target = st.text_input("Enter Target Domain/IP", placeholder="127.0.0.1", key="scan_target")
            ...
            with st.spinner(f"Scanning {target}..."):
                ...
                if results.get("errors"):
                    ...
                    if results["ports"]:
                        ...
                        if not df.empty:
                            df = df[["number", "protocol", "state", "service", "product", "version"]]
                            st.table(df)
                        ...
                        if isinstance(ai, dict):
                            ...
                            if "potential_vulnerabilities" in ai:
                                for v in ai["potential_vulnerabilities"]:
                                    with st.expander(f"⚠️ {v.get('type', 'Vulnerability')}", expanded=True):
...
def show_exploit():
    ...
    target = st.text_input("Enter Target", placeholder="127.0.0.1", key="exploit_target")
    
    col1, col2 = st.columns(2)
    with col1:
        use_ai = st.checkbox("Enable AI Orchestration", value=True, key="exploit_use_ai")
    ...
    if st.button("💀 Run Exploit Pipeline"):
        if not target:
            ...
            async def run_exploit():
                ...
                results = await st.session_state.exploit.run_exploit_pipeline(target, recon_data, use_ai=use_ai)
                return results
                    ...
                    if results["vulnerabilities"]:
                        with st.expander("🛡️ Identified Vulnerabilities", expanded=True):
                            for vuln in results["vulnerabilities"]:
                                st.markdown(f"**{vuln.get('cve_id')}** - {vuln.get('affected_software')}")
                        ...
                        with st.expander("🐚 Generated Exploits", expanded=True):
                            for exploit in results["exploits"]:
                                if "error" not in exploit:
                                    st.success(f"Exploit generated: {exploit.get('file_path')}")
                                    st.code(exploit.get("code", ""), language="python")
                                else:
                                    st.error(f"Generation error: {exploit.get('error')}")
                ...
                async def get_response():
                    ...
                    if use_search:
                         with st.status("🌍 Searching the web..."):
                             search_results = await st.session_state.vulnforge.ai_analyzer.perform_web_search(prompt)
                             final_prompt = f"Context from web search:\n{search_results}\n\nUser Question: {prompt}"
                    
                    return await st.session_state.vulnforge.ai_analyzer.ollama.generate(final_prompt)
                ...
                try:
                    ...
                    if response:
                        ...
                        if not st.session_state.vulnforge.ai_analyzer.ollama.ai_enabled:
                            ...
                            st.caption("Reason: Ollama service might be unreachable or models missing.")
                except Exception as e:
