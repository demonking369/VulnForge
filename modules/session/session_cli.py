...
class SessionCLI:
    ...
    def cmd_list(self, args):
        ...
        for session in sessions:
            ...
            status_display = f"{status_emoji.get(session['status'], '')} {session['status']}"
    ...
    def cmd_load(self, args):
        ...
        try:
            ...
            self.console.print(f"[cyan]Status:[/cyan] {session_data['session']['status']}")
            self.console.print(f"[cyan]Mode:[/cyan] {session_data['session']['mode']}")
            
        except FileNotFoundError:
            self.console.print(f"[bold red]✗ Session not found:[/bold red] {args.session_id}")
    ...
    def cmd_resume(self, args):
        ...
        try:
            ...
            if progress.get('total_steps', 0) > 0:
                ...
                self.console.print(f"[cyan]Progress:[/cyan] {percentage}% ({progress['completed_steps']}/{progress['total_steps']} steps)")
...
[bold]Progress:[/bold] {progress.get('percentage', 0)}% ({progress.get('completed_steps', 0)}/{progress.get('total_steps', 0)} steps)
    ...
    def cmd_export(self, args):
        ...
        try:
            ...
            serializer.export_session(session_data, export_path, include_data=args.include_data)
...
def setup_session_parser(subparsers):
    ...
    new_parser.add_argument('--mode', choices=['offensive', 'defensive'], default='offensive', help='Operational mode')
    ...
    list_parser.add_argument('--status', choices=['active', 'paused', 'completed'], help='Filter by status')
    list_parser.add_argument('--mode', choices=['offensive', 'defensive'], help='Filter by mode')
    ...
    resume_parser.add_argument('session_id', nargs='?', help='Session ID (uses last active if not provided)')
    ...
    # session status
    status_parser = session_subparsers.add_parser('status', help='Show current session status')
    ...
    export_parser.add_argument('--include-data', action='store_true', help='Include session data directory')
