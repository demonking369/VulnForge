    ...
    async def discover_subdomains(self, target: str) -> List[str]:
        ...
        try:
            ...
            cmd = [
                ...
                "-d", domain,
                "-silent",
                "-sources", "crtsh,alienvault,hackertarget,digitorus,anubis"
            ]
            ...
            if not stdout:
                ...
                print(f"Warning: No output from subfinder for {domain}. Check if subfinder is installed and configured correctly.")
                return [domain]  # Return main domain if no subdomains found
            
            # Process output
            subdomains = [line.strip() for line in stdout.decode().splitlines() if line.strip()]
            
            if not subdomains:
                self.logger.warning(f"No subdomains found for {domain}. Check subfinder installation, network, and API keys.")
                print(f"Warning: No subdomains found for {domain}. Check subfinder installation, network, and API keys.")
                return [domain]  # Return main domain if no subdomains found
            ...
            return [target.split("://")[-1].split("/")[0]]  # Return main domain on error 